...
wavfile_input = 'wavfiles/00131.wav' # wavfile to process - 00131 corresponds to the Flemish sentence 'David draag drie gele boten'
# Input parameters
...
if save_wav:
    ...
    sio_wav.write(save_wav + '/' + os.path.basename(wavfile_input)[:-4] + '_20k.wav',int(fs_model),stim[0,:,0]*10**(-(wav_dBref+20*np.log10(p0))/20))
...
for modeli, modeldir in enumerate(modeldirs):
    ...
    if save_wav:
        sio_wav.write(save_wav + '/' + os.path.basename(wavfile_input)[:-4] + '_' + dirtitle + '_20k.wav',int(fs_model),stimp[modeli,0,:,0]*10**(-(wav_dBref+20*np.log10(p0))/20))
